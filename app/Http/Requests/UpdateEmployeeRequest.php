...
class UpdateEmployeeRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        if ($user->email !== $this->input('email') && ! $this->input('password')) {
            $passwordRule = 'required|confirmed|min:8';
            $passwordConfirmRule = 'required|in:'.$this->input('password');
        } else {
            $passwordRule = 'nullable|confirmed|min:8';
            $passwordConfirmRule = 'nullable|in:'.$this->input('password');
        }
        ...
        return [
            ...
            'email' => 'email|max:255|unique:users,email,'.$this->employee.'id',
            'password' => $passwordRule,
            'password_confirmation' => $passwordConfirmRule,
